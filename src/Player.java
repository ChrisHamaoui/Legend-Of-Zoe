...
public class Player extends Character {
    ...
    public void move(int x, int y, Level lvl) {
        ...
        if (x < LevelGenerator.LARGEUR && x > -1 && y < LevelGenerator.HAUTEUR && y > -1
                ...
                && (getNbOfHexaforces() == Level.getLevel()? true: !(gameBoard[y][x] instanceof Exit))) {
     ...
     *  @param lvl: Holds the level in which the player is about to perform an attack in.
     */
    public void attack(Level lvl) {
        ...
        for (int i = 0; i < surroundings.length; i++) {
            if (surroundings[i] != null) { // Check surrounding coordinates
                ...
                for (int j = 0; j < lvl.getOverlappingObjs().length; j++) {
                    GameObject obj = lvl.getOverlappingObjs()[j];
    ...
    public void lootItem(Enemy enemy) {
        ...
        switch (enemy.getItem()) {
            ...
            case ("potionvie"):
                ...
                break;
            case ("hexaforce"):
                setNbOfHexaforces(getNbOfHexaforces() + 1);
                enemy.setLooted(true);
    ...
    public void playerInput(Level lvl) {
        ...
        for (int i = 0; i < playerInput.length(); i++) {
            switch (playerInput.charAt(i)) {
                case ('w'): 
                    move(getX(), getY() - 1, lvl);
                    takePortal(lvl);
                    setPlayed(true);
                    lvl.enemyMovement();
                    break;
                case ('a'):
                    move(getX() - 1, getY(), lvl);
                    takePortal(lvl);
                    setPlayed(true);
                    lvl.enemyMovement();
                    break;
    ...
    public void takePortal(Level lvl) {
        ...
        if (getNbOfHexaforces() == Level.getLevel() && portalSurroundings(lvl)) {
            Level nextLevel = new Level();

            lvl.setGameBoard(nextLevel.getGameBoard()); // Create a new level
            lvl.setOverlappingObjs(nextLevel.getOverlappingObjs()); // Add new objs to overlappingObjs
            System.out.println("You've entered LEVEL " + Level.getLevel());
            ...
            for (int i = 0; i < nextLevel.getOverlappingObjs().length; i++) {
                if (nextLevel.getOverlappingObjs()[i] instanceof Exit) {
                    lvl.getExit().setX(((Exit) nextLevel.getOverlappingObjs()[i]).getX());
                    lvl.getExit().setY(((Exit) nextLevel.getOverlappingObjs()[i]).getY());
                }
            }
    ...
    /**
     * Checks the player's surrounding coordinates to see if there's a portal nearby.
     * @param lvl: The level the player is currently playing in.
     * @return: A boolean value (true) if there's a portal in the player's surroundings.
     */
    public boolean portalSurroundings(Level lvl) {
